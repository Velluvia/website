...
export async function POST(req: NextRequest) {
  try {
    ...
    // Anthropic's API requires the conversation to start with a "user" message.
    // The widget always seeds its local UI with an assistant greeting bubble
    // (so visitors see something before typing), and that greeting is included
    // in every request's message history — strip any leading non-user messages
    // here so the API call is always valid, regardless of what the client sends.
